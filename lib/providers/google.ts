...
//   1. The project-aware planner writes each shot prompt.
//   2. Gemini image model renders a first-frame keyframe that matches the
//      requested project style (photorealistic, 3D cartoon, commercial, etc.).
//   3. Veo animates the keyframe with native audio.
//   4. Finished MP4 is re-hosted on fal storage for browser playback + merge.
// ---------------------------------------------------------------------------
...
function gHeaders() { return { "x-goog-api-key": cleanEnv("GEMINI_API_KEY") ?? "", "Content-Type": "application/json" }; }
async function gFetch(url: string, init?: RequestInit) {
  ...
  if (!res.ok) throw new Error(`Google video request failed (HTTP ${res.status}).`);
  return res;
}
...
async function generateKeyframe(input: ShotInput): Promise<{ data: string; mimeType: string } | null> {
  ...
  try {
    const res = await gFetch(`${G_BASE}/models/${model}:generateContent`, {
      method: "POST",
      body: JSON.stringify({
        contents: [{ parts: [{ text: `First-frame reference keyframe for a vertical 9:16 video. Match the requested visual style exactly and preserve all identity/continuity instructions. ${input.prompt} Absolutely avoid: ${input.negativePrompt}.` }] }],
...
export class GoogleVeoProvider implements VideoProvider {
  ...
  readonly configurationHint = "Set GEMINI_API_KEY (Veo access) and FAL_KEY (clip hosting + merge). Optional: GOOGLE_VIDEO_MODEL, GOOGLE_IMAGE_MODEL.";
  private videoModel = cleanEnv("GOOGLE_VIDEO_MODEL") ?? "veo-3.1-fast-generate-preview";
  get name() { return `Google (project keyframe → ${this.videoModel}, native audio)`; }
  get configured() { return Boolean(cleanEnv("GEMINI_API_KEY") && cleanEnv("FAL_KEY")); }
  ...
  async submitShot(input: ShotInput) {
    ...
    const res = await gFetch(`${G_BASE}/models/${this.videoModel}:predictLongRunning`, {
      method: "POST",
      body: JSON.stringify({ instances: [instance], parameters: { aspectRatio: "9:16", durationSeconds: seconds, negativePrompt: input.negativePrompt } }),
    });
  ...
  async getShotResult(operationName: string): Promise<ShotResult> {
    const res = await gFetch(`${G_BASE}/${operationName}`);
    const op = (await res.json()) as { response?: { generateVideoResponse?: { generatedSamples?: Array<{ video?: { uri?: string } }> }; generatedVideos?: Array<{ video?: { uri?: string } }> } };
  ...
  async cancelShot(operationName: string) { void operationName; }
}
